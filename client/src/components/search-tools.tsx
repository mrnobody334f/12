...
import { Settings2, Clock, Globe, FileText, X } from "lucide-react";
    ...
    <div className="flex items-center gap-2 flex-wrap">
        ...
        <PopoverContent className="w-80" align="start" data-testid="popover-search-tools">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-sm text-foreground">Search tools</h3>
              {hasActiveFilters && (
                <Button
                  variant="ghost"
                  size="sm"
      ...
      <AnimatePresence>
        {timeFilter !== "any" && (
          ...
          >
            <Badge variant="secondary" className="gap-1" data-testid="badge-active-time-filter">
              <Clock className="h-3 w-3" />
        ...
        {languageFilter !== "any" && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 0.8 }}
          >
            <Badge variant="secondary" className="gap-1" data-testid="badge-active-language-filter">
              <Globe className="h-3 w-3" />
              {languageOptions.find(o => o.value === languageFilter)?.label}
              <button
                onClick={() => onLanguageFilterChange("any")}
                className="ml-1 hover:bg-muted rounded-sm p-0.5"
                data-testid="button-remove-language-filter"
              >
                <X className="h-3 w-3" />
              </button>
            </Badge>
        ...
        {fileTypeFilter !== "any" && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 0.8 }}
          >
            <Badge variant="secondary" className="gap-1" data-testid="badge-active-file-type-filter">
              <FileText className="h-3 w-3" />
